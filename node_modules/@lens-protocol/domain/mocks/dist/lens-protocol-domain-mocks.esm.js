...
import { mockEvmAddress, mock32BytesHexString, mockUsdcAmount, mockDaiAmount } from '@lens-protocol/shared-kernel/mocks';
import { mock } from 'jest-mock-extended';
import { P as Profile, R as ReportReason } from '../../dist/ReportClaim-2d81b360.esm.js';
...
import { F as FollowPolicyType } from '../../dist/FollowPolicy-1ce77379.esm.js';
...
function mockICredentials(overrides) {
...
function mockNftOwnershipChallenge() {
  return {
...
function mockIUnsignedVote(overrides) {
  return _objectSpread2({
    pollId: mockPollId()
  }, overrides);
}
function mockISignedVote(overrides) {
  return _objectSpread2({
    pollId: mockPollId(),
...
function mockVotePollRequest(overrides) {
  return _objectSpread2({
    pollId: mockPollId(),
    choice: 0
  }, overrides);
}
function mockCreateUnsignedVoteRequest(overrides) {
  return _objectSpread2({
    pollId: mockPollId(),
    choice: 0,
    voter: mockWallet()
...
function mockIUnsignedVoteFactory(_ref) {
...
function mockIProfileTransactionGateway(_ref) {
  var request = _ref.request,
    result = _ref.result;
  var profileTransactionGateway = mock();
  when(profileTransactionGateway.createProfileTransaction).calledWith(request).mockResolvedValue(result);
...
function mockNoFeeFollowConfig(overrides) {
  return _objectSpread2({
...
function mockProveNftOwnershipRequest() {
  return {
    contractAddress: mockEvmAddress(),
...
function mockNftOwnershipSignature(overrides) {
  return _objectSpread2({
    id: faker.datatype.uuid(),
    signature: mockSignature()
  }, overrides);
}
function mockINftOwnershipChallengeGateway(_ref2) {
...
function mockIPayTransactionGateway(_ref4) {
  var request = _ref4.request,
    wallet = _ref4.wallet,
    unsignedTransaction = _ref4.unsignedTransaction;
  var gateway = mock();
  when(gateway.createUnsignedTransaction).calledWith(request, wallet).mockResolvedValue(unsignedTransaction);
  return gateway;
}

export { MockedDataTransaction, MockedMetaTransaction, MockedNativeTransaction, mockActiveWallet, mockAnyBroadcastingError, mockAnyTransactionRequestModel, mockAnyoneReferencePolicyConfig, mockAppId, mockBlockProfilesRequest, mockChallenge, mockChargeFollowConfig, mockClaimHandleRequest, mockCollectFee, mockCreateCommentRequest, mockCreateMirrorRequest, mockCreatePostRequest, mockCreateProfileRequest, mockCreateQuoteRequest, mockCreateUnsignedVoteRequest, mockDelegableProtocolTransactionRequestModel, mockFreeFollowRequest, mockHidePublicationRequest, mockIBalanceGateway, mockICredentials, mockIDelegatedTransactionGateway, mockIMetaTransactionNonceGateway, mockIMomokaRelayer, mockINftOwnershipChallengeGateway, mockIOnChainRelayer, mockIPayTransactionGateway, mockIProfileTransactionGateway, mockISignedMomokaGateway, mockISignedOnChainGateway, mockISignedProtocolCall, mockISignedVote, mockITokenGateway, mockITransactionCompletionPresenter, mockIUnsignedProtocolCall, mockIUnsignedVote, mockIUnsignedVoteFactory, mockJustWalletLoginRequest, mockLegacyCollectRequest, mockLinkHandleRequest, mockMultirecipientCollectRequest, mockNftOwnershipChallenge, mockNftOwnershipSignature, mockNoFeeFollowConfig, mockNonce, mockPaidFollowRequest, mockPollId, mockProfile, mockProfileId, mockProfileLoginRequest, mockProtocolTransactionRequestModel, mockProtocolTransactionRequestModelWithOffChainFlag, mockProveNftOwnershipRequest, mockPublicationId, mockReportPublicationRequest, mockSetProfileMetadataRequest, mockSignature, mockSimpleCollectRequest, mockTogglePropertyRequest, mockTokeAvailability, mockTokenAllowanceRequest, mockTokenAvailabilityRequest, mockTransactionData, mockTransactionError, mockTransactionHash, mockTransactionQueue, mockUnblockProfilesRequest, mockUnfollowRequest, mockUnknownActionRequest, mockUnknownOpenActionConfig, mockUnlinkHandleRequest, mockUnsignedTransaction, mockUpdateFollowPolicyRequest, mockUpdateProfileManagersRequest, mockVotePollRequest, mockWallet };
