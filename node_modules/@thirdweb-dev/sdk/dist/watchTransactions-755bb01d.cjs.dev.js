...
async function getContractFromAbi(params) {
  ...
  const [resolvedAddress, {
    ...
    SmartContract
  }] = await Promise.all([index.resolveAddress(params.address), provider.getNetwork(), Promise.resolve().then(function () { return require('./smart-contract-49d36504.cjs.dev.js'); })]);
...
async function getContract(params) {
  ...
  const [resolvedAddress, {
    ...
    ContractPublisher
  }] = await Promise.all([index.resolveAddress(params.address), provider.getNetwork(), Promise.resolve().then(function () { return require('./contract-publisher-ff36ccb6.cjs.dev.js'); }).then(function (n) { return n.contractPublisher; })]);
  if (inContractCache(resolvedAddress, chainId)) {
    return getCachedContract(resolvedAddress, chainId);
  }
