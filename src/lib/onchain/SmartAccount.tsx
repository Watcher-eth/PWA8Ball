...
import { UsdcAbi, UsdcAddress } from "@/constants/contracts/usdc";
...
export function SmartAccountProvider({
  ...
  async function connectSmartAccount() {
    ...
    const allowance = await publicClient.readContract({
      ...
      abi: UsdcAbi,
      args: [smartAccountClient.account?.address!, UsdcAddress[8453]],
    ...
    if (allowance < 1n) {
      ...
      try {
        ...
        const hash = await contract.write.approve([UsdcAddress[8453], 10000000000n]);
        console.log("hash", hash);
      } catch (error) {
        console.error("Failed to approve usdc smart wallet:", error);
        // throw error
      }
    }
  ...
  }, [isConnected, userReady, authenticated, smartAccountReady]);
